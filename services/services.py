...
class UserPersonalService:
    ...
    def download_personal_file_list(self, user):
        ...
        folder_list = Folder.objects.filter(owner=user.id, personal_folder=True)
    ...
    def upload_personal_file_list(self, user, data):
        ...
        for current_obj in personal_file_list:
            current_file = File.objects.get(id=current_obj['_id'], owner=user.id)
            current_file.owner = user.id
            current_file.upload_date = datetime.fromtimestamp(current_obj['upload_date'])
            current_file.parent = str(current_obj['parent'])
            fixed_file_list.append(current_file)

        for current_obj in personal_folder_list:
            current_folder = Folder.objects.get(id=current_obj['_id'], owner=user.id)
            current_folder.owner = user.id
            current_folder.created_at = datetime.fromtimestamp(current_obj['created_at'])
            current_folder.updated_at = datetime.fromtimestamp(current_obj['updated_at'])
            fixed_folder_list.append(current_folder)

        File.objects.bulk_update(fixed_file_list, ['_id', 'upload_date', 'owner', 'parent'])
        Folder.objects.bulk_update(fixed_folder_list, ['owner', 'created_at', 'updated_at'])
...
class FolderService:
    ...
    def get_folder_sublist(self, user_id, folder_id):
        ...
        for current_sub_folder_id in subfolder_list:
            if current_sub_folder_id == "/":
                ...
                current_folder = folderUtils.get_folder_info(current_sub_folder_id, user_id)
    ...
    def get_folder_list(self, user, query):
        ...
        parent = None if query.get("parent", None) == "/" else query.get("parent", None)
    ...
    def get_subfolder_full_list(self, user, folder_id):
        ...
        while True:
            ...
            current_sub_folders = self.get_folder_list(user, {"parent": current_id.id})
    ...
    def move_folder(self, user_id, folder_id, parent_id):
        ...
        folder = folderUtils.move_folder(folder_id, user_id, parent_id, ''.join(parent_list))
        if not folder:
            raise Exception("Move Folder Not Found")
        folder_children = folderUtils.find_all_folders_by_parent(folder_id, user_id)
        for current_folder_child in folder_children:
            current_folder_child_parent_list = current_folder_child.parent_list
            index_of_folder_id = current_folder_child_parent_list.index(folder_id)
            current_folder_child_parent_list = current_folder_child_parent_list[index_of_folder_id:]
        ...
        for current_file_child in file_children:
            ...
            index_of_folder_id = current_file_child_parent_list.index(folder_id)
            ...
            fileUtils.move_file(current_file_child.id, user_id, current_file_child.parent, ",".join(current_file_child_parent_list))
...
class UserService:
    ...
    def create(self, user_data):
            ...
            user.save()
            refresh = RefreshToken.for_user(user)
            email_token = jwt.encode({"_id":user.id}, "EMAIL-VER",  algorithm='HS256')
    ...
    def verify_email(self, verify_token):
        try:
            decoded = jwt.decode(verify_token, 'EMAIL-VER', algorithms=['HS256'])
            user_id = decoded['_id']
            user = User.objects.get(id=user_id)
    ...
    def send_password_reset(self, email):
        try:
            user = User.objects.get(email=email)
            password_reset_token = jwt.encode({"_email":email}, "PASSWORD-RESET",  algorithm='HS256')
    ...
    def reset_password(self, verify_token, new_password):
        try:
            decoded = jwt.decode(verify_token, 'PASSWORD-RESET', algorithms=['HS256'])
...
class MongoFileService:
    ...
    def get_quick_list(self, user):
        file_list = File.objects.filter(owner_id=user.id).order_by("-upload_date")[:10]
        file_list = json.loads(serializers.serialize('json', file_list))
        return file_list

    def get_public_download(self, temp_token):
        payload = jwt.decode(temp_token, "MAKE-PUBLIC",algorithms=['HS256'])
    ...
    def make_public(self, user_id, file_id):
        try:
            file = File.objects.get(id=file_id, owner_id=user_id)
            token = jwt.encode({"id": str(user_id), "file_id":file.id, "s3_id": file.s3_id}, "MAKE-PUBLIC", algorithm='HS256')
            file.link = token
    ...
    def make_one_time_public(self, user_id, file_id):
        try:
            ...
            token = jwt.encode({"_id": str(user_id), "file":file_id}, "ONE-TIME-PUBLIC", algorithm='HS256')
            file.link = token
            file.link_type = "one"
            file.save()
            return token
    ...
    def get_file_info(self, user_id, file_id):
        try:
            ...
            if parent_id != "/":
                parent_folder = Folder.objects.filter(owner=user_id, id=parent_id).first()
                if parent_folder:
    ...
    def get_list(self, user, query):
        ...
        file_list = json.loads(serializers.serialize('json', file_list))[start_at:limit]
        return file_list
    ...
    def remove_temp_token(self, user, temp_token, current_uuid):
        ...
        decoded = jwt.decode(temp_token, settings.PASSWORD_ACCESS, algorithms=['HS256'])
    ...
    def get_suggested_list(self, user_id, search_query):
        search_query = search_query
        file_list = File.objects.filter(owner__id=user_id, filename__icontains=search_query)
        folder_list = Folder.objects.filter(owner__id=user_id, name__icontains=search_query)
        file_list = json.loads(serializers.serialize('json', file_list))
